...
// --- Global State untuk Mengontrol Animasi Kereta ---
const activeAnimations = new Map(); 

// ...
// ... di dalam client.addEventHandler(async (event) => { ... })
// ... dan di dalam switch (cmd.toLowerCase()) { ... }

case `${PrefixsCommand}kreta`: {
	const activeAnimations = new Map();
    if (activeAnimations.has(message.chatId)) {
        return await client.sendMessage(message.chatId, {
            message: "<blockquote>⚠️ Animasi kereta sudah berjalan di chat ini. Gunakan <b>.stopkreta</b> untuk menghentikannya!</blockquote>",
            parseMode: "html",
            replyTo: message.id
        });
    }
    ...
    // Kirim pesan awal
    const sent = await client.sendMessage(message.chatId, {
        message: `<pre>${frames[0]}\n${track}</pre>`,
        parseMode: "html"
    });

    const animationState = { running: true, index: 0, messageId: sent.id, chatId: message.chatId };
    activeAnimations.set(message.chatId, animationState);

    // Fungsi loop animasi
    const animate = async () => {
        while (activeAnimations.get(message.chatId)?.running) {
            const state = activeAnimations.get(message.chatId);
            state.index = (state.index + 1) % frames.length;
            const currentFrame = frames[state.index];

            const text = `<pre>${currentFrame}\n${track}</pre>

<i>Gunakan <b>.stopkreta</b> untuk menghentikan.</i>`;
            ...
            try {
                ...
                // Jika pesan tidak bisa diedit (misal chat sudah ditutup)
                console.log("Animasi Kereta dihentikan karena error edit:", e.message);
                activeAnimations.delete(message.chatId);
                return;
            }

            // Tunggu 200 milidetik sebelum frame berikutnya
...
case `${PrefixsCommand}stopkreta`: {
    if (activeAnimations.has(message.chatId)) {
        const state = activeAnimations.get(message.chatId);
        state.running = false; // Set flag stop
        activeAnimations.delete(message.chatId); // Hapus dari map

        // Edit pesan terakhir untuk konfirmasi berhenti
        await client.editMessage(state.chatId, {
            message: state.messageId,
            text: `<blockquote>🚂💨💨💨 Animasi Kereta dihentikan.</blockquote>`,
            parseMode: "html"
        }).catch(() => {}); // Abaikan error jika pesan sudah dihapus

        await client.sendMessage(message.chatId, {
            message: "<blockquote>✅ Animasi kereta berhasil dihentikan.</blockquote>",
            parseMode: "html",
            replyTo: message.id
        });
    } else {
        await client.sendMessage(message.chatId, {
            message: "<blockquote>❌ Tidak ada animasi kereta yang berjalan di chat ini.</blockquote>",
            parseMode: "html",
            replyTo: message.id
        });
    }
...
// ... (kasus-kasus command lainnya) ...

case `${PrefixsCommand}gamemenu`: {
    const text = `<blockquote>
🎮 <b>Juno Official Game Menu</b>

Silakan pilih game yang tersedia.

╭────── 「 MINIGAMES 」──────
├ ${RandomMode(simbol)} <b>.xox</b> &lt;@user&gt;
├ 
╰─────────────

╭────── 「 OFFICIAL BOTS 」──────
├ ${RandomMode(simbol)} <a href="https://t.me/tictactoe">TicTacToe Bot</a>
├ ${RandomMode(simbol)} <a href="https://t.me/MasterpieceBot">Chess Bot</a>
├ ${RandomMode(simbol)} <a href="https://t.me/Snakes_and_Ladders_bot">Snakes & Ladders Bot</a>
╰─────────────

<i>💡 Gunakan <b>.xox @user</b> untuk menantang seseorang di chat ini!</i>
</blockquote>`;
    await client.sendMessage(message.chatId, { message: text, parseMode: "html" });
    break;
}

case `${PrefixsCommand}xox`: {
    const peer = await resolvePeer(client, message);
    if (!peer) return;

    if (xoxGames.has(message.chatId)) {
        return await client.sendMessage(message.chatId, {
            message: "<blockquote>⚠️ Ada game XOX yang sedang berjalan di chat ini. Akhiri dulu atau tunggu!</blockquote>",
            parseMode: "html",
            replyTo: message.id
        });
    }

    const targetUsernameOrId = arg.trim();
    if (!targetUsernameOrId) {
        return await client.sendMessage(message.chatId, {
            message: "<blockquote>⚠️ Format: <b>.xox @username</b> atau ID untuk menantang!</blockquote>",
            parseMode: "html",
            replyTo: message.id
        });
    }
    ...
    let targetEntity;
    try {
        targetEntity = await client.getInputEntity(targetUsernameOrId);
    } catch {
        return await client.sendMessage(message.chatId, {
            message: `<blockquote>❌ Pengguna <b>${escapeHtml(targetUsernameOrId)}</b> tidak ditemukan.</blockquote>`,
            parseMode: "html",
            replyTo: message.id
        });
    }

    const targetUser = await client.getEntity(targetEntity);
    const targetId = String(targetUser.id);
    const senderId = String(message.senderId);

    if (targetId === senderId) {
        return await client.sendMessage(message.chatId, {
            message: "<blockquote>❌ Tidak bisa menantang diri sendiri!</blockquote>",
            parseMode: "html",
            replyTo: message.id
        });
    }

    const senderName = message.sender.firstName || "Player 1";
    const targetName = targetUser.firstName || "Player 2";
    ...
    xoxGames.set(message.chatId, initialState);

    const keyboard = createXOXKeyboard(initialState.board);
    const turnName = senderId === initialState.currentTurnId ? initialState.playerXName : initialState.playerOName;

    const gameMessage = await client.sendMessage(message.chatId, {
        message: `
<b>❌ O X ⭕ Tic-Tac-Toe Dimulai!</b>

<b>❌ Player X:</b> ${escapeHtml(senderName)}
<b>⭕ Player O:</b> ${escapeHtml(targetName)}

<b>Giliran:</b> ❌ ${escapeHtml(turnName)} (X)
`,
        parseMode: "html",
        replyTo: message.id,
        buttons: keyboard
    ...
    // Simpan ID pesan untuk edit selanjutnya
    initialState.messageId = gameMessage.id;
    xoxGames.set(message.chatId, initialState);
...
function createXOXKeyboard(board) {
    const keyboard = [];
    for (let i = 0; i < 3; i++) {
        const row = [];
        for (let j = 0; j < 3; j++) {
            const index = i * 3 + j;
            const text = board[index] === ' ' ? ' ' : board[index];
            row.push({ text: text, callback_data: `xox_move_${index}` });
        }
        keyboard.push(row);
    }
    keyboard.push([{ text: "❌ Akhiri Game", callback_data: `xox_end` }]);
...
// Handler untuk Callback Query (Inline Buttons XOX)
client.addEventHandler(async (event) => {
    if (!event.hasOwnProperty('query')) return; // Pastikan ini adalah callback query
    const query = event.query;
    const data = query.data;
    const chatId = query.peer?.chatId ?? query.peer?.channelId ?? query.peer?.userId;
    if (!chatId) return;

    if (!data.startsWith('xox_')) return;

    const game = xoxGames.get(chatId);
    if (!game) {
        return event.answer({ text: "Game sudah berakhir atau tidak ditemukan.", showAlert: true });
    }
    
    const callerId = String(query.senderId);

    // --- Penanganan Akhiri Game ---
    if (data === 'xox_end') {
        if (callerId !== game.playerXId && callerId !== game.playerOId && callerId !== String(settings.ownerId)) {
             return event.answer({ text: "Anda bukan pemain atau owner untuk mengakhiri game ini.", showAlert: true });
        }
        xoxGames.delete(chatId);
        await client.editMessage(chatId, {
            message: game.messageId,
            text: `<blockquote>❌ Game Tic-Tac-Toe diakhiri oleh ${query.user.firstName || "Seseorang"}.</blockquote>`,
            parseMode: "html",
            buttons: null
        }).catch(() => {});
        return event.answer({ text: "Game diakhiri.", showAlert: false });
    }
    
    // --- Penanganan Gerakan (Move) ---
    if (!data.startsWith('xox_move_')) return;

    if (callerId !== game.currentTurnId) {
        return event.answer({ text: "Bukan giliran Anda!", showAlert: true });
    }

    const cellIndex = parseInt(data.split('_')[2]);
    if (game.board[cellIndex] !== ' ') {
        return event.answer({ text: "Kotak ini sudah terisi!", showAlert: true });
    }

    // Lakukan Gerakan
    game.board[cellIndex] = game.currentTurnSymbol;
    const winner = checkWinner(game.board);
    let newKeyboard = createXOXKeyboard(game.board);
    let newText = "";

    if (winner === 'draw') {
        newText = `
<b>❌ O X ⭕ DRAW! Permainan Selesai.</b>
Semua kotak terisi! Tidak ada pemenang.
`;
        xoxGames.delete(chatId);
        newKeyboard = null;
    } else if (winner) {
        const winnerName = winner === 'X' ? game.playerXName : game.playerOName;
        newText = `
<b>🎉 WINNER: ${winner} (${escapeHtml(winnerName)})!</b>
Permainan Selesai!
`;
        xoxGames.delete(chatId);
        newKeyboard = null;
        ...
        // Ganti Giliran
        game.currentTurnSymbol = game.currentTurnSymbol === 'X' ? 'O' : 'X';
        game.currentTurnId = game.currentTurnId === game.playerXId ? game.playerOId : game.playerXId;
        const turnName = game.currentTurnId === game.playerXId ? game.playerXName : game.playerOName;
        
        newText = `
<b>❌ O X ⭕ Tic-Tac-Toe</b>

<b>❌ Player X:</b> ${escapeHtml(game.playerXName)}
<b>⭕ Player O:</b> ${escapeHtml(game.playerOName)}

<b>Giliran:</b> ${game.currentTurnSymbol === 'X' ? '❌' : '⭕'} ${escapeHtml(turnName)} (${game.currentTurnSymbol})
`;
        xoxGames.set(chatId, game); // Update state
    }

    // Edit pesan dan beri notifikasi
    await client.editMessage(chatId, {
        message: game.messageId,
        text: newText,
        parseMode: "html",
    ...
    event.answer({ text: `Anda bergerak ke kotak ${cellIndex + 1}.`, showAlert: false });

}, new Telegram.events.CallbackQuery()); // Gunakan Telegram.events.CallbackQuery

// End Handler Callback Query
